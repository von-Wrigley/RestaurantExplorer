...
export const getFavoriteDish= async(q:string)=> {
 ...
 let newFavorites 
  ...
  const {data:dataFav, error} =await supabase.from('profiles').select('favorite_dish').eq('id', user?.id).single()
  ...
  const {data, error:errorAdd} =await supabase.from('profiles').update({'favorite_dish':newFavorites }).eq('id', user?.id)
   if(errorAdd){
