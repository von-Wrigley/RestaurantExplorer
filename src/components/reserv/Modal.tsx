...
function Modal({
  ...
  const [, formAction, isPending] = useActionState(makeReserv, null);
  ...
  const availablesTables = new Map();
  ...
  const handleBooking = async (id: string, x: string) => {
    ...
    setSetTable(Array.from(availablesTables));
  };
  ...
  return createPortal(
    <div className="bg-white border-amber-300  border-2 rounded-2xl z-10 absolute top-6/12 p-5 right-1/2">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-row gap-8">
          <div className="flex flex-col gap-4 ">
            {transDates.map((x, index) => (
              <button
                onClick={() => handleBooking(id, x)}
                key={index}
                className="p-2 text-2xl  hover:cursor-pointer self-start bg-amber-300 rounded-sm hover:bg-violet-500 focus:bg-violet-500"
              >
        ...
        <form
          action={formAction}
          className="p-3 flex flex-col border-amber-300 border rounded-2xl gap-3"
        >
          <p>Выбранная дата: {getTime.dt} </p>
          <p>Выбранное время: {getTime.tm} </p>
          <input type="hidden" name="selected_time" value={getTime.tm} />
          <input type="hidden" name="selected_date" value={getTime.dt} />
          <input type="hidden" name="res_id" value={id} />
          <input type="hidden" name="table_id" value={getTable} />
          <textarea
            id="special_request"
            name="special_request"
            defaultValue="Особые пожелания. Например, предоставить вазу если вы придете с цветами"
            className=" focus:ring focus:ring-violet-500   focus:outline-none    border border-amber-300 focus:border-violet-500 p-2 rounded-sm"
          ></textarea>
          <button
            type="submit"
            className="bg-violet-500 hover:cursor-pointer p-1 px-1.5 rounded-sm self-center-safe w-fit"
          >
            {' '}
            {isPending ? 'Бронируем... ' : 'Зарезервировать'}
          </button>
        </form>
      </div>
    </div>,
