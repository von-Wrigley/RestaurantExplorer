...
function BusinessHours({register}) {
  ...
  return (
    ...
    <div className=" bg-gray-300  ring shadow-xl ring-gray-900/5 w-fit  h-fit p-5 rounded-sm">
 ...
 <tr key={day.id} className="flex flex-row justify-end gap-10  my-2">
      ...
      <td><input {...register(`business_hours.${day.id}.open`)} type="text" className="rounded-md border bg-white w-fit"  /></td>
      <td><input {...register(`business_hours.${day.id}.close`)} type="text" className="border rounded-md bg-white w-fit"/></td>  
    </tr>
   )
