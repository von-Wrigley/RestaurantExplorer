...
function UploadImaagestoStorage({register, id, setValue}) {
 ...
 for (const image of imagesAr){
   ...
   const fullPathName = `${id}/${image?.name}`
      const {  error:erStorUpload } = await supabase
  .storage
  .from('resPhoto')
...
const { data:Iamgurl } = supabase
  .storage
  .from('resPhoto')
  .getPublicUrl('fullPathName')
 ...
 console.log(allURL)
  ...
  return (
    <div className="col-start-3  bg-gray-300  ring shadow-xl ring-gray-900/5 rounded-sm">
        ...
        <input  className="bg-white w-full" type='file' accept="image/*" multiple onChange={handlefileChange}  />
