  ...
  return (
    <div className="flex flex-col bg-gray-300  ring shadow-xl ring-gray-900/5 p-3.5">
      <h3>Выберите вид кухни </h3>
        ...
        <div>
          {typesCuisineRu.map(x=> (
            <label key={x} className="flex flex-row gap-1" >
              <input type="checkbox" checked={field.value.includes(x)} onChange={(e)=> {
                       let checkedCusines= []
                       if(e.target.checked){
       ...
       {errors.translatable?.ru?.cuisines?.message && <p className='text-red-500'>{errors.translatable?.ru?.cuisines?.message}</p>}
    </div>
  )
}
